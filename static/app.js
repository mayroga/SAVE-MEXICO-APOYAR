...
async function api(url,opciones={},timeout=15000){
    ...
    try{
        ...
        if(!r.ok){
            const msg=data&&data.detail?data.detail:"El servidor rechazó la solicitud.";
            throw new Error(msg);
        }
        return data;
...
function validarCheck(){
    ...
    ["btn-plan-diario","btn-plan-mensual"].forEach(id=>{if($(id))$(id).disabled=!chk?.checked;});
}

function calcularEdadAutomaticamente(valor){
    const m=String(valor||"").match(/\d{4}/);
    if(!m)return;
    const ano=parseInt(m[0],10),hoy=new Date();
    let edad=hoy.getFullYear()-ano;
    const partes=String(valor).match(/\d{1,4}/g)||[];
    if(partes.length>=3){
        let a=parseInt(partes[0]),b=parseInt(partes[1]),c=parseInt(partes[2]);
        let y=a>1900?a:c,mes=a>1900?b:b,dia=a>1900?c:a;
        if(y>1900&&mes>=1&&mes<=12&&dia>=1&&dia<=31){
            const nacimiento=new Date(y,mes-1,dia);
            if(nacimiento.getMonth()===mes-1&&nacimiento.getDate()===dia){
                edad=hoy.getFullYear()-y;
                if(hoy.getMonth()<mes-1||(hoy.getMonth()===mes-1&&hoy.getDate()<dia))edad--;
            }
        }
    }
    if(edad>0&&edad<120){
        $("input-edad").value=edad;
...
function activarMicrofono(id){
    ...
    if(!SR){alert("El dictado por voz no está soportado en este dispositivo.");return;}
    if(reconocimientoActual)try{reconocimientoActual.stop()}catch(_){}
    const r=new SR();
    reconocimientoActual=r;r.lang="es-MX";r.interimResults=false;r.maxAlternatives=1;
    const boton=$("btn-voz-"+id.split("-")[1]);
    r.onstart=()=>{if(boton)boton.innerText="🛑 Escuchando...";};
    r.onresult=e=>{
...
function iniciarMecanismoRevision(){
    const nombre=$("input-nombre").value.trim(),fecha=$("input-fecha").value.trim();
    const edad=$("input-edad").value.trim(),origen=$("input-origen").value;
    const direccion=$("input-direccion").value.trim(),tel=$("input-tel").value.trim();
    const edo=$("input-estado").value;
    if(!nombre||!fecha||!edad||!direccion){
        alert("Por favor rellena todos tus datos obligatorios.");return;
    }
    perfil={nombre_completo:nombre,fecha_nacimiento:fecha,edad,origen_mexico:origen,
        direccion_usa:direccion,telefono:tel,estado:edo,nacionalidad:"mexicana"};
    try{localStorage.setItem("perfil_retenido",JSON.stringify(perfil));}catch(_){}
    $("resumen-visual-datos").innerHTML="";
    [["Tu Nombre",nombre],["Tu Fecha de Nacimiento",`${fecha} (${edad} años)`],
     ["Tu Dirección en USA",direccion],["Tu Teléfono",tel||"No indicado"],
     ["Tu Estado Seleccionado",edo]].forEach(x=>{
        const p=document.createElement("p"),b=document.createElement("strong");
        b.textContent=x[0]+": ";p.append(b,document.createTextNode(x[1]));$("resumen-visual-datos").appendChild(p);
    });
    contadorRevisiones=0;actualizarAlertaYVozRevision();irAPaso("paso-triple-revision");
}
...
async function cargarCatalogo(){
    ...
    try{
        ...
        (datos||[]).forEach(t=>{
            ...
            strong.textContent=t.nombre;span.textContent=t.descripcion;
            btn.append(strong,span);btn.onclick=()=>iniciarTramite(t.caso);
            lista.appendChild(btn);
        });
        irAPaso("paso-tramites");
    }catch(e){
        alert("No se pudo cargar el catálogo. "+e.message);
    }finally{peticionActiva=false;}
}

async function iniciarTramite(caso){
    if(peticionActiva)return;
    casoActual=caso;respuestas={};peticionActiva=true;
    try{
        ...
        procesarPaso(data);
    }catch(e){alert("Error al iniciar el trámite. "+e.message);}
    finally{peticionActiva=false;}
}
...
async function enviarRespuesta(idPregunta,valor){
    ...
    try{
        const data=await api("/api/continuar",{
            method:"POST",headers:{"Content-Type":"application/json"},
            body:JSON.stringify({caso:casoActual,pregunta_id:idPregunta,
                respuesta:valor,respuestas,perfil})
        });
        procesarPaso(data);
    }catch(e){
...
function procesarPaso(data){
    if(data.pregunta){
        ...
        const cont=$("contenedor-opciones");cont.innerHTML="";
        (data.pregunta.opciones||[]).forEach(o=>{
            const lbl=document.createElement("label"),radio=document.createElement("input");
            lbl.className="radio-label";radio.type="radio";radio.name="r_opt";radio.value=o;
    ...
    }else alert("La respuesta del servidor está incompleta.");
}

function mostrarResultado(r){
    resultadoFinal=r;irAPaso("paso-resultado");
    const caja=$("res-caja-estado");
    caja.className="box-info "+(r.estado==="LISTO PARA TU CITA"?"success":"danger");
    caja.innerHTML="";
    const b=document.createElement("strong");b.textContent=`ESTATUS: ${r.estado}`;
    caja.append(b,document.createElement("br"),document.createTextNode(r.mensaje_estado||""));
    $("res-consulado").innerText=r.consulado_nombre||"";
    $("res-direccion").innerText="📍 "+(r.consulado_direccion||"");
    $("res-telefono").innerText="📞 Tel central: "+(r.consulado_telefono||"");
    $("res-pago").innerText=r.pago_estimado||"";
    $("res-cita").innerText=r.cita_estatus||"";
    $("res-cita").style.color=(r.cita_estatus||"").includes("PENDIENTE")?"var(--danger)":"var(--accent)";
    inyectarLista("res-requisitos",r.requisitos_oficiales);
    $("seccion-faltantes").style.display=(r.faltantes||[]).length?"block":"none";
    ...
    if(btnWeb&&r.url_consulado){
        btnWeb.onclick=()=>window.open(r.url_consulado,"_blank","noopener,noreferrer");
    }
}
...
async function verVistaPrevia(){
    ...
    try{
        const res=await fetch("/api/pdf-preview",{
            method:"POST",headers:{"Content-Type":"application/json"},
            credentials:"same-origin",body:JSON.stringify(resultadoFinal)
        });
    ...
    }catch(e){alert("No se pudo generar la vista previa. "+e.message);}
    finally{peticionActiva=false;}
}

function cerrarVistaPrevia(){
    const iframe=$("pdf-frame"),modal=$("modal-pdf");
    if(iframe){const u=iframe.src;iframe.src="";if(u.startsWith("blob:"))URL.revokeObjectURL(u);}
...
async function descargarPDF(){
    ...
    try{
        ...
        a.href=url;a.download=`Hoja_de_Ruta_${(casoActual||"consular").toUpperCase()}.pdf`;
        document.body.appendChild(a);a.click();a.remove();
        setTimeout(()=>URL.revokeObjectURL(url),1000);
    }catch(e){alert("No se pudo guardar tu Hoja de Ruta. "+e.message);}
    finally{peticionActiva=false;}
}
...
function comenzarDeNuevoLimpio(){
    ...
    try{
        ...
        if(d){
            perfil=d;
            $("input-nombre").value=d.nombre_completo||"";
            $("input-fecha").value=d.fecha_nacimiento||"";
            $("input-edad").value=d.edad||"";
            $("input-direccion").value=d.direccion_usa||"";
            $("input-tel").value=d.telefono||"";
            $("input-origen").value=d.origen_mexico||"Michoacán";
            $("input-estado").value=d.estado||"Otro";
        }
    }catch(_){perfil={};}
    $("check-legal").checked=false;validarCheck();irAPaso("paso-legal");
}
...
async function bypassDesarrollador(user,pass){
    if(!user||!pass){alert("Por favor escribe tu usuario y contraseña.");return;}
    try{
        ...
        if(data.ok){
            ...
            $("muro-pago-stripe").style.display="none";
            $("btn-comenzar").style.display="block";
            alert("¡Acceso de desarrollador aprobado!");
            irAPaso("paso-datos");
        }
    }catch(e){alert("No fue posible validar el acceso. "+e.message);}
}
...
async function solicitarAccesoStripe(plan){
    ...
    try{
        const data=await api(`/api/checkout?plan=${encodeURIComponent(plan)}`,{method:"POST"},20000);
...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    if(fecha)fecha.addEventListener("blur",()=>calcularEdadAutomaticamente(fecha.value));
    validarCheck();
    comprobarAcceso();
