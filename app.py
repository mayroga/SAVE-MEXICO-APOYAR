...
def construir_pdf(r,editable=True):
 ...
 y=texto_pdf(
  c,
  "DOCUMENTO PRELIMINAR. REVISA, CORRIGE Y CONFIRMA TU INFORMACION.",
  y,9,True
 )-8
 ...
 bloques=[
  ...
  ("10. INFORMACIÓN ESPECIAL",r.get("especial")),
  ("11. QUÉ DEBES HACER",r.get("acciones")),
  ("12. PAGO",r.get("pago")),
  ("13. CITA",r.get("cita")),
  ("14. VIGENCIA",r.get("vigencia")),
  ("15. ENTREGA",r.get("entrega"))
 ]
 ...
 y-=5
 y=seccion(c,"16. INFORMACIÓN OFICIAL",y)
...
def extraer_pdf(data):
 try:
  ...
  raise HTTPException(
   400,
   "No fue posible leer el PDF. Verifica que el archivo tenga texto y que pypdf esté instalado."
  )
...
async def api_documento(file:UploadFile=File(...)):
 ...
 return {
  ...
  "mensaje":
   "PDF leído correctamente. Revisa la información antes de utilizarla."
   if texto else
   "El PDF no contiene texto extraíble. Puedes copiar la información manualmente."
 }
...
def api_pdf_preview(x:ResultadoRequest):
 try:
  ...
  return StreamingResponse(
   ...
   headers={
    "Content-Disposition":
    'inline; filename="Hoja_de_Ruta_PRELIMINAR.pdf"',
...
def api_pdf(x:ResultadoRequest):
 try:
  ...
  return StreamingResponse(
   ...
   headers={
    "Content-Disposition":
    'attachment; filename="Hoja_de_Ruta_EDITABLE.pdf"',
    "Cache-Control":"no-store"
   }
  )
 except Exception as e:
