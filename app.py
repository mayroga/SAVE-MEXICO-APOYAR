...
for p in (STATIC, DATA, OUT):
    p.mkdir(parents=True, exist_ok=True)

ACCESS_FILE = DATA / "access.json"
MEMORY_FILE = DATA / "asesor_mexicano_tramites.json"
...
def admin_cookie(r):
    t = r.cookies.get("save_admin_token")
    rec = load_access().get("admin_tokens", {}).get(htoken(t)) if t else None
    return bool(rec and rec.get("active"))

def customer_access(r):
    t = r.cookies.get("save_access_token")
    rec = load_access().get("tokens", {}).get(htoken(t)) if t else None
    if not rec or not rec.get("active", True):
        return None
    try:
        if rec.get("expires") and datetime.now(timezone.utc).timestamp() > float(rec["expires"]):
            return None
    except:
        return None
    return rec
...
def load_memory():
    return loadj(MEMORY_FILE)

def save_memory(d):
    savej(MEMORY_FILE, d)

def memory_key(c, m=""):
    return key(c) + (" | " + key(m) if key(m) else "")

def get_memory(c, m=""):
    return load_memory().get(memory_key(c, m))

def save_tramite_memory(c, m, profile):
    d = load_memory()
    d[memory_key(c, m)] = {
        "categoria_tramite": clean(c),
        "modalidad": clean(m),
        "perfil": profile,
...
def construir_perfil(c, m="", contexto="", datos=None):
    datos = datos or {}
    old = get_memory(c, m)
    mem = old.get("perfil", {}) if old else {}
...
La estructura PDF debe ser propia de este trámite y modalidad. No copies otro trámite."""
    profile = ai_json(p)
    save_tramite_memory(c, m, profile)
    return profile
...
def admin_login(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    r.set_cookie("save_admin_token", activate_admin(), max_age=86400, httponly=True, secure=True, samesite="lax")
    return r
...
def verify_payment(session_id: str):
    ...
    try:
        s = stripe.checkout.Session.retrieve(session_id, expand=["subscription", "line_items"])
    except Exception as e:
    ...
    if s.mode == "subscription":
        ...
        expires = now + 86400
    raw = activate(email, plan, s.id, subid, expires)
    r = JSONResponse({"ok": True, "email": email, "plan": plan})
    r.set_cookie("save_access_token", raw, max_age=max(3600, int(expires - now)), httponly=True, secure=True, samesite="lax")
    return r
...
async def stripe_webhook(request: Request):
    ...
    if typ in ("checkout.session.completed", "checkout.session.async_payment_succeeded"):
        ...
        if email:
            ...
            if sub:
                try:
                    ss = stripe.Subscription.retrieve(sub if isinstance(sub, str) else sub.id)
        ...
        try:
            if cid:
                email = stripe.Customer.retrieve(cid).get("email")
        except:
            pass
        if email:
            d = load_access()
...
def perfil_tramite(req: PerfilTramiteRequest, _ = Depends(require_access)):
    if not clean(req.categoria_tramite):
        ...
        try:
            r = gemini.models.generate_content(
                ...
                contents=[
                    ...
                    ASESOR_SYSTEM,
                    f"Extrae solamente información que realmente aparezca en este documento. Identifica tipo de documento, nombres, fechas, números, lugares, datos del trámite y documentos mencionados. Si algo no puede leerse: NO LEGIBLE.\nTEXTO:{text[:40000]}"
                ]
            )
            t = (r.text or "").strip()
            if t:
...
def generar_pdf(data, content):
    ...
    title(content.get("titulo") or data.categoria_tramite)
    wrap(content.get("subtitulo") or "Documento privado de preparación", 10, 14)
    y -= 8

    section("IDENTIFICACIÓN DEL TRÁMITE")
    item("Trámite", data.categoria_tramite)
    item("Modalidad", data.modalidad)

    section("DATOS DEL SOLICITANTE")
    item("Nombre", nombre_completo(data))
    item("Fecha de nacimiento", data.fecha_nacimiento)
    item("Lugar de nacimiento", data.lugar_nacimiento)
    item("Nacionalidad", data.nacionalidad)
    item("Domicilio en Estados Unidos", data.direccion_usa)
    item("Teléfono", data.telefono)
    item("Correo", data.correo)
    item("Consulado", data.consulado)
    item("Cita", data.cita)

    for sec in content.get("secciones", []):
        if safe(sec.get("titulo")):
            section(sec["titulo"])
        for it in sec.get("items", []):
            val = safe(it.get("valor"))
            tipo = safe(it.get("tipo"))
            if tipo == "confirmar":
                val = val or "POR CONFIRMAR CON LA AUTORIDAD"
    ...
    if content.get("checklist"):
        ...
        for x in content["checklist"]:
            item("[ ]", x)

    if content.get("confirmaciones"):
        section("DEBE CONFIRMARSE")
        for x in content["confirmaciones"]:
            item("CONFIRMAR", x)

    section("ESTADO DEL TRÁMITE")
    item("Estado", data.estado_posterior or "PENDIENTE")
    item("Información adicional", data.extra_1)
...
def generar_guia(data: DatosTramite, _ = Depends(require_access)):
    ...
    if not clean(data.categoria_tramite):
        raise HTTPException(400, "Debe indicar el trámite.")
