...
"grupos":[["pasaporte"],["primera vez","primera","sacar","obtener","solicitar","nuevo"]],
"preguntas":[
...
"pago":"Confirma el monto y la forma de pago de la tarifa consular vigente.",
"fuente":FUENTES["pasaporte"]
}},

{
"id":"pasaporte_renovacion","servicio":"cita","titulo":"Renovar mi pasaporte",
"grupos":[["pasaporte"],["renovar","renovacion","renovarlo","vencido","vencer","expirado","caducado"]],
"preguntas":[
q("menor","¿El pasaporte es para una persona menor de 18 años?",["Sí","No"]),
...
"grupos":[["pasaporte"],["perdi","perdido","perdida","robo","robado","robaron","extraviado","destruido","mutilado"]],
"preguntas":[
...
{
"id":"pasaporte_menor","servicio":"cita","titulo":"Pasaporte para un menor",
...
("Identificaciones correspondientes de los padres","padres_id")
],
"resultado":{
...
{
"id":"pasaporte_extemporanea","servicio":"cita","titulo":"Mi acta puede ser extemporánea",
...
{
"id":"pasaporte_conyuge","servicio":"cita","titulo":"Apellido de mi cónyuge en el pasaporte",
...
{
"id":"pasaporte_naturalizado","servicio":"cita","titulo":"Pasaporte de una persona mexicana por naturalización",
...
{
"id":"matricula_primera_adulto","servicio":"cita","titulo":"Sacar mi matrícula consular",
...
("Documento ORIGINAL de nacionalidad mexicana","nacionalidad"),
("Identificación ORIGINAL con fotografía","identidad"),
("Comprobante ORIGINAL de domicilio con dirección completa","domicilio")
],
"resultado":{
...
"grupos":[["matricula"],["renovar","renovacion","vencida","vencer"]],
"preguntas":[
...
("Comprobante de domicilio actual","domicilio")
],
"resultado":{
...
{
"id":"matricula_perdida","servicio":"cita","titulo":"Perdí o me robaron mi matrícula",
...
{
"id":"matricula_menor","servicio":"cita","titulo":"Matrícula consular para un menor",
...
"grupos":[["hijo","hija","nino","niño","nina","niña","bebe"],["nacimiento","nacio"],["mexicano","mexicana"]],
"preguntas":[
...
("Identificaciones correspondientes","identidad")
],
"resultado":{
...
"pago":"Confirma si existe algún costo aplicable a tu situación.",
...
"documentos":[("Información necesaria para obtener la copia certificada del acta","registrada")],
"resultado":{
...
def buscar_casos(texto,servicio):
 ...
 for c in CASOS:
  if c["servicio"]==servicio:
   s=puntuar(c,texto)
   if s>=2:a.append((s,c))
 return sorted(a,key=lambda x:x[0],reverse=True)
...
def fusionar_contexto(res,texto,pregunta_id=""):
 ...
 if pregunta_id and texto.strip():r[pregunta_id]=texto.strip()
 return r
...
def inferir(caso,res,texto=""):
 ...
 if p.get("menor")=="Sí" and caso:
  if any(x["id"]=="menor" for x in caso.get("preguntas",[])):r.setdefault("menor","Sí")

 if p.get("naturalizacion")=="Sí" and caso:
  if any(x["id"]=="naturalizado" for x in caso.get("preguntas",[])):r.setdefault("naturalizado","Sí")
 ...
 if re.search(r"\bperdi\b|\brobaron\b|\brobo\b|\bextravi",n):
  if caso and caso["id"]=="pasaporte_perdido":r.setdefault("reporte","No estoy seguro")
 return r

def pregunta_actual(caso,res):
 for qx in caso.get("preguntas",[]):
  v=res.get(qx["id"])
...
def requisitos_dinamicos(caso,res):
 ...
 if cid=="pasaporte_extemporanea":
  try:
   ...
   r=int(re.search(r"\d{4}",str(res.get("ano_registro",""))).group())
   edad=r-n
  ...
  except: a.append("Confirma los años exactos de nacimiento y registro.")
 ...
 if cid=="matricula_menor":
  a.append("Deben revisarse nacionalidad, identidad, padres/tutores y autorizaciones.")
...
def estado_preparacion(caso,res):
 ...
 if any(x.startswith("NO VAYAS") for x in esp):return "rojo"
 if any(x["estado"]=="falta" for x in documentos(caso,res)):return "amarillo"
 if any(x.startswith("ATENCIÓN") for x in esp):return "rojo"
 return "verde"
...
def resultado(caso,res):
 ...
 if nivel=="rojo":estado="ATENCIÓN: revisa esto antes de acudir."
 elif nivel=="amarillo":estado="TE FALTA ALGO: revisa los puntos pendientes."
 else:estado="PARECES LISTO: no aparece un requisito crítico faltante en tus respuestas."
 ...
 cid=caso["id"]
 es_pas=cid.startswith("pasaporte")
 es_mat="matricula" in cid
 ...
 "checklist":{
  ...
  "falta":falta,
  "revisar":revisar,
  "documentos":[x["nombre"] for x in ds]
 },
 "documentos":ds,
 ...
 "originales":["Presenta los documentos ORIGINALES que correspondan a tu caso."],
 "copias":["Lleva copias únicamente cuando la fuente oficial las indique."],
 "ruta":{
  "tramite":caso["titulo"],
  "llevar_original":"Presenta los documentos ORIGINALES que correspondan a tu caso.",
  "copias":"Lleva copias únicamente cuando la fuente oficial las indique.",
  "cita":FUENTES["citas"] if any(x["id"]=="cita" for x in caso["preguntas"]) else "",
  "pago":r["pago"]
 },
 "pago":r["pago"],
 "revision":"Revisa cuidadosamente todos los datos antes de firmar o antes de imprimir el documento.",
 "vigencia":"La vigencia del pasaporte depende de la edad: menores de 3 años, 1 o 3 años; de 3 a menores de 18, 3 o 6 años; adultos, 3, 6 o 10 años." if es_pas else ("La matrícula consular tiene una vigencia de cinco años." if es_mat else ""),
 "entrega":"La información oficial indica entrega el mismo día una vez cumplidos los requisitos; puede tomar alrededor de dos horas si no hay fallas del sistema." if es_pas or es_mat else "",
 "importante":[
...
def interpretar(servicio,texto,respuestas=None):
 ...
 cid=res.get("_caso")
 if cid:
  caso=obtener_caso(cid)
  if not caso:return iniciar(servicio)
  if res.get("_pregunta_id"):
   res[res["_pregunta_id"]]=texto
