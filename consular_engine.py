...
def incierto(x):
 n=normalizar(x)
 return any(z in n for z in ("no estoy seguro","no se","no lo se","no recuerdo","tal vez","quizas"))
...
"confirma":"Revisa cuidadosamente los datos antes de firmar.",
...
q("cita","¿Ya tienes tu cita?",["Sí","No"])],"documentos":[("Pasaporte que vas a renovar","pasaporte_anterior")],"resultado":{
...
{"id":"pasaporte_extemporanea","servicio":"cita","titulo":"Mi acta puede ser extemporánea","grupos":[["acta","nacimiento"],["extemporanea","registro tarde","registraron tarde"]],"preguntas":[
...
q("cita","¿Ya tienes tu cita?",["Sí","No"])],"documentos":[("Prueba complementaria de nacimiento","prueba")],"resultado":{
...
q("cita","¿Ya tienes tu cita?",["Sí","No"])],"documentos":[("Acta de matrimonio","acta_matrimonio")],"resultado":{
...
{"id":"pasaporte_naturalizado","servicio":"cita","titulo":"Pasaporte de una persona mexicana por naturalización","grupos":[["pasaporte"],["naturalizado","naturalizacion","carta de naturalizacion"]],"preguntas":[
...
{"id":"matricula_primera_adulto","servicio":"cita","titulo":"Sacar mi matrícula consular","grupos":[["matricula"],["primera","sacar","obtener","solicitar"]],"preguntas":[
...
{"id":"matricula_renovacion","servicio":"cita","titulo":"Renovar mi matrícula consular","grupos":[["matricula"],["renovar","renovacion","vencida","vencer"]],"preguntas":[
...
{"id":"matricula_perdida","servicio":"cita","titulo":"Perdí o me robaron mi matrícula","grupos":[["matricula"],["perdi","perdido","robaron","robo","robada","extraviada"]],"preguntas":[
...
{"id":"matricula_menor","servicio":"cita","titulo":"Matrícula consular para un menor","grupos":[["matricula"],["menor","hijo","hija","niño","niña","nino","nina"]],"preguntas":[
...
{"id":"registro_nacimiento_extranjero","servicio":"documento","titulo":"Registrar a mi hijo como mexicano","grupos":[["hijo","hija","nino","niño","nina","niña","bebe"],["nacimiento","nacio"],["mexicano","mexicana"]],"preguntas":[
...
q("identidad","¿Tienes las identificaciones correspondientes?",SI)],"documentos":[
...
{"id":"acta_nacimiento_mexicana","servicio":"documento","titulo":"Necesito mi acta de nacimiento mexicana","grupos":[["acta"],["nacimiento","nacido","nacida"],["mexicana","mexicano","mexico","copia","certificada"]],"preguntas":[q("registrada","¿Tu nacimiento ya está registrado en México?",["Sí","No","No estoy seguro"])],"documentos":[("Información necesaria para obtener la copia certificada del acta","registrada")],"resultado":{
...
q("identidad","¿Tiene identificación oficial vigente?",SI)],"documentos":[("Identificación oficial vigente","identidad")],"resultado":{
...
def buscar_casos(texto,servicio):
 return sorted([(puntuar(c,texto),c) for c in CASOS if c["servicio"]==servicio and puntuar(c,texto)>=2],key=lambda x:x[0],reverse=True)

def identificar(servicio,texto):
 a=buscar_casos(texto,servicio)
 if not a:return None
 if len(a)==1 or a[0][0]>=a[1][0]+2:return a[0][1]
 return [x[1] for x in a[:4]]
...
def extraer_perfil(texto,perfil=None):
 ...
 m=re.search(r"(?:nombre|me llamo)\s*[:\-]?\s*([A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:\s+[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+){1,5})(?:[,.]|$)",raw,re.I)
...
def inferir(caso,res,texto=""):
 ...
 if p.get("menor")=="Sí" and any(x["id"]=="menor" for x in caso.get("preguntas",[])):r.setdefault("menor","Sí")
 if p.get("naturalizacion")=="Sí" and any(x["id"]=="naturalizado" for x in caso.get("preguntas",[])):r.setdefault("naturalizado","Sí")
 if re.search(r"\bya tengo cita\b|\btengo cita\b|\bcita confirmada\b",n):r.setdefault("cita","Sí")
 if re.search(r"\bno tengo cita\b|\bsin cita\b",n):r.setdefault("cita","No")
 if re.search(r"\btengo mi pasaporte\b|\btengo el pasaporte\b",n):r.setdefault("pasaporte_anterior","Sí")
 return r

def debe_preguntar(caso,x,res):
 cid=caso["id"];i=x["id"]
 if cid in ("pasaporte_menor","matricula_menor") and i=="op7" and afirmativo(res.get("padres")):return False
 if cid=="pasaporte_menor" and i=="sentencia" and negativo(res.get("padres")):return True
 return True
...
def pregunta_actual(caso,res):
 for x in caso.get("preguntas",[]):
  ...
  v=res.get(x["id"])
  if v is None or not str(v).strip():return x
 return None
...
def respuesta_pregunta(caso,x,res):
 return {
  ...
  "opciones":x.get("opciones",[]),"respuestas":res,
...
def requisitos_dinamicos(caso,res):
 ...
 if cid=="pasaporte_primera_vez":
  if afirmativo(res.get("acta_extemporanea")):a.append("Revisa si necesitas prueba complementaria por posible acta extemporánea.")
  if afirmativo(res.get("conyuge")):a.append("Puede corresponder presentar acta de matrimonio.")
 if cid=="pasaporte_renovacion":
  if any(afirmativo(res.get(x)) for x in ("antiguo","emergencia","restriccion","un_ano")):a.append("ATENCIÓN: tu pasaporte puede requerir un procedimiento diferente a una renovación normal.")
  if afirmativo(res.get("naturalizado")):a.append("Revisa el requisito de la carta de naturalización.")
 if cid=="pasaporte_perdido":
  a+=["Debes contar con el reporte o acta de la autoridad competente.","El procedimiento parte de los requisitos de una primera expedición."]
 if cid=="pasaporte_menor":
  ...
  if negativo(res.get("padres")):a.append("Si falta un padre, revisa el procedimiento OP-7.")
  if afirmativo(res.get("sentencia")):a.append("Lleva la resolución judicial correspondiente.")
 if cid=="pasaporte_extemporanea":
  try:
   ...
   if edad>limite:a.append("NO VAYAS TODAVÍA: el registro puede ser extemporáneo y requerir prueba complementaria.")
  except:pass
 if cid=="pasaporte_conyuge" and afirmativo(res.get("acta_matrimonio")):a.append("Lleva el acta de matrimonio correspondiente.")
 if cid=="pasaporte_naturalizado":a.append("Lleva la carta de naturalización ORIGINAL o copia certificada cuando corresponda.")
 if cid=="matricula_primera_adulto":a.append("Debes acreditar nacionalidad, identidad y domicilio.")
 if cid=="matricula_renovacion" and afirmativo(res.get("domicilio_cambio")):a.append("Si cambiaste de residencia, lleva comprobante del domicilio actual.")
 if cid=="matricula_perdida":a.append("Debes llenar el formulario donde explicas la pérdida o robo.")
 if cid=="matricula_menor":a.append("Deben revisarse nacionalidad, identidad, padres/tutores y autorizaciones.")
...
def estado_preparacion(caso,res):
 e=requisitos_dinamicos(caso,res);d=documentos(caso,res)
 if any(x.startswith("NO VAYAS") for x in e):return "rojo"
 if any(x.startswith("ATENCIÓN") for x in e):return "rojo"
 if any(x["estado"]=="falta" for x in d):return "amarillo"
 return "verde"
...
def resultado(caso,res):
 ...
 r=caso["resultado"];cid=caso["id"]
 pas=cid.startswith("pasaporte");mat="matricula" in cid
 ...
 return {
  "estado":"resuelto","tipo":"resultado","caso":cid,"titulo":caso["titulo"],"tramite":caso["titulo"],
  "prepara":r["prepara"],"confirma":r["confirma"],"pago":r["pago"],"fuente":r["fuente"],"fuente_oficial":r["fuente"],
  "respuestas":res,"perfil":dict(res.get("_perfil") or {}),"nivel":nivel,"estado_texto":estado,
  "checklist":{"tiene":tiene,"falta":falta,"revisar":revisar,"documentos":[x["nombre"] for x in ds]},
  "documentos":ds,"especiales":esp,"personas_obligatorias":[],
  ...
  "copias":[],
  "revision":"Revisa cuidadosamente todos los datos antes de firmar o antes de imprimir el documento.",
  "vigencia":"La vigencia del pasaporte depende de la edad: menores de 3 años, 1 o 3 años; de 3 a menores de 18, 3 o 6 años; adultos, 3, 6 o 10 años." if pas else "La matrícula consular tiene una vigencia de cinco años." if mat else "",
...
def iniciar(servicio,caso_id=None,respuestas=None):
 ...
 if caso_id:
  return seleccionar(caso_id,respuestas)
 return {
  "estado":"seleccionar","tipo":"catalogo","catalogo":True,"servicio":servicio,
  "pregunta":"¿Qué trámite necesitas preparar?","pregunta_id":"tramite",
  "opciones":catalogo(servicio),"respuestas":dict(respuestas or {})
 }

def seleccionar(caso_id,respuestas=None):
 caso=obtener_caso(caso_id)
 if not caso:return {"estado":"error","mensaje":"Trámite no encontrado."}
...
def continuar(caso_id,respuestas=None,pregunta_id="",texto=""):
 ...
 if not caso:return {"estado":"error","mensaje":"Trámite no encontrado."}
 res=dict(respuestas or {});res["_caso"]=caso_id
 if pregunta_id and str(texto).strip():res[pregunta_id]=str(texto).strip()
...
def interpretar(servicio,texto,respuestas=None,pregunta_id=""):
 ...
 if not texto:return iniciar(servicio,respuestas)

 encontrados=identificar(servicio,texto)

 if not encontrados:
  return iniciar(servicio,res)

 if isinstance(encontrados,list):
  return {
   "estado":"seleccionar","tipo":"seleccionar_caso","catalogo":True,"servicio":servicio,
